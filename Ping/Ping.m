...
-(void)setTimeout:(NSTimeInterval)timeout {
    @synchronized(self) {
        if (self.isPinging) {
            if (self.debug) {
                NSLog(@"Ping: can't set timeout while pinger is running.");
            }
        }
        else {
            _timeout = timeout;
...
-(void)setTtl:(NSUInteger)ttl {
    @synchronized(self) {
        if (self.isPinging) {
            if (self.debug) {
                NSLog(@"Ping: can't set ttl while pinger is running.");
            }
        }
        else {
            _ttl = ttl;
...
-(void)setPayloadSize:(NSUInteger)payloadSize {
    @synchronized(self) {
        if (self.isPinging) {
            if (self.debug) {
                NSLog(@"Ping: can't set payload size while pinger is running.");
            }
        }
        else {
...
-(void)setPingPeriod:(NSTimeInterval)pingPeriod {
    @synchronized(self) {
        if (self.isPinging) {
            if (self.debug) {
                NSLog(@"Ping: can't set pingPeriod while pinger is running.");
            }
        }
        else {
...
-(void)listenOnce {
    ...
    if (bytesRead > 0) {
        ...
        if([host isEqualToString:self.hostAddressString]) { // only make sense where received packet comes from expected source
            ...
            NSNumber *key = @(seqNo);
            PingSummary *pingSummary = [(PingSummary *)self.pendingPings[key] copy];
            ...
            if (pingSummary) {
                if ([self isValidPingResponsePacket:packet]) {
                    ...
                    pingSummary.status = PingStatusSuccess;
                    ...
                    pingSummary.status = PingStatusFail;
...
-(void)sendPing {
    if (self.isPinging) {
        ...
        PingSummary *newPingSummary = [PingSummary new];
        ...
        if (self.socket == 0) {
            ...
            newPingSummary.status = PingStatusPending;
            ...
            PingSummary *pingSummaryCopy = [newPingSummary copy];
            ...
            NSTimer *timeoutTimer = [NSTimer scheduledTimerWithTimeInterval:self.timeout
                                                                     target:[NSBlockOperation blockOperationWithBlock:^{
                                                                         ...
                                                                         newPingSummary.status = PingStatusFail;
        ...
        //failed to send
            ...
            PingSummary *pingSummaryCopyAfterFailure = [newPingSummary copy];
...
-(id)init {
    if (self = [super init]) {
        self.setupQueue = dispatch_queue_create("Ping setup queue", 0);
        self.isStopped = YES;
